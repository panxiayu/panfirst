...
staff.forEach(s => {
    if (s.name) {
        const pinyin = getNamePinyin(s.name);
        db.prepare('UPDATE staff SET name_pinyin = ? WHERE id = ?').run(pinyin, s.id);
        count++;
    }
});
console.log('Updated', count, 'staff records');

// Verify
const sample = db.prepare('SELECT id, name, name_pinyin FROM staff LIMIT 5').all();
